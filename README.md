...
truncate anything beyond the model's context window (2048 tokens for the
`nomic-embed-text` build Ollama ships) rather than reject it, so long notes
get a vector from their opening rather than none at all.
