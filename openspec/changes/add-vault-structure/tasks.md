...
- [x] 18.1 Run the full server and app test suites; fix any cross-phase regressions — after rebasing onto latest `origin/main` (18 upstream commits), all suites green with no fixes needed: server 914/914, shared 41/41, app 222/222
- [x] 18.2 ~~Manually walk the golden path in a dev build~~ — walked the golden path against a real running server (`dart_frog build` + the compiled binary, per `make run-server`) via direct API calls rather than pixel-level browser clicks: the Flutter web setup screen's pre-existing (unrelated, pre-change) `https://`-only guard blocks a plain-`http://` loopback dev server, and Chrome's automation layer cannot click through the resulting self-signed-cert interstitial to work around it with a local TLS proxy — confirmed both a code bypass and a browser-trust workaround were the wrong call here and reverted/abandoned them. Verified end-to-end instead: created `Projects/Project Alpha.md`, created `Projects/Alpha/Meeting Notes.md` with `[[Project Alpha]]` + `#urgent`, confirmed `GET /notes/{id}/backlinks` resolved it; renamed "Project Alpha" → "Project Alpha Renamed" and confirmed the referencing note's link text and version were rewritten (`grep`+`GET` on disk and via API); confirmed `#urgent` appears in `GET /tags` and `GET /notes?tag=urgent`; moved "Meeting Notes" to `Archive/` and confirmed the file moved on disk, `GET /notes/tree` dropped the now-empty `Projects/Alpha` folder and added `Archive`, and `GET /notes?path=Archive` returned it. All of this exercises the same `NoteWriteService`/`Storage`/`MetaIndex`/`LinkIndex` code paths the UI calls into; the UI's own rendering of this data is covered by the Flutter widget test suite added in sections 14-17. **Found and fixed a real bug in the process**: `GET /tags` was being shadowed by the SPA static-file fallback whenever the web bundle is served (the normal production mode) because the search+MCP phase added the route without adding it to `static_web_middleware.dart`'s API-path allowlist — fixed by adding a `Routes.tags` constant and registering it, with a regression test (see the `fix(server): stop the SPA static bundle from shadowing GET /tags` commit)
- [x] 18.3 Update `server/API.md` and `server/STORAGE.md` to describe the new path/link/tag surface
- [x] 18.4 Commit: `docs(server): document vault paths, links, and tags`
