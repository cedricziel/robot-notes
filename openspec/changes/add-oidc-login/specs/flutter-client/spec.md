...
- **GIVEN** the user has entered a server URL whose `/.well-known/oauth-authorization-server` document contains `robotnotes_oidc_login_supported: true`, on a desktop or web build
- **WHEN** the setup screen loads that server's capabilities
...
- **GIVEN** the user has entered a server URL whose discovery document omits `robotnotes_oidc_login_supported`, or the app is running on a mobile platform
...
- **WHEN** the user chooses "Sign in", completes the provider's login in the opened browser, and is redirected back
