...
**Native `RunnerTests` runner for iOS, plain `flutter test` for macOS.** Confirmed via Flutter's own `integration_test` docs and its example app: iOS screenshot capture needs a **hosted unit-test bundle** target (`TEST_HOST` pointed at `Runner.app`, using the `INTEGRATION_TEST_IOS_RUNNER` Objective-C macro) driven via `flutter drive` — not a UI Testing Bundle/XCUITest target as originally assumed, and not `flutter test` directly, which cannot capture screenshots on iOS at all. In this repo, `flutter create`'s current template already scaffolds an empty `RunnerTests` unit-test target (`TEST_HOST` wired, `Podfile` already has `target 'RunnerTests' do inherit! :search_paths end`) — only its placeholder `RunnerTests.swift` needed replacing with an Objective-C `RunnerTests.m` implementing the macro. macOS doesn't have this restriction — `flutter test integration_test/screenshot_test.dart -d macos` captures directly, no native changes needed.
