...
- **NEW** native `RunnerTests` runner (`app/ios/RunnerTests/RunnerTests.m`, a hosted unit-test bundle per `integration_test`'s iOS support, not a UI Testing Bundle) so iOS screenshot capture can run via `flutter drive` (plain `flutter test` cannot capture screenshots on iOS); macOS capture runs via plain `flutter test -d macos`
