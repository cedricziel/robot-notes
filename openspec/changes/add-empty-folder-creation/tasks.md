...
## 5. Docs and end-to-end check

- [ ] 5.1 Run the full server and app test suites; fix any regressions
- [ ] 5.2 Update `server/STORAGE.md` to document the `.folder` marker file convention alongside the existing `<id>.md`/frontmatter documentation
- [ ] 5.3 Update `server/API.md` to document `POST /notes/tree`
- [ ] 5.4 Manually verify against a running server + app: create an empty folder from the sidebar, confirm it appears with a zero count, restart the server, confirm it's still listed; create a note inside it and confirm the marker file and note coexist on disk
- [ ] 5.5 Commit: `docs(server): document the empty-folder marker file and POST /notes/tree`
