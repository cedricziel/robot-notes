...
- **[Trade-off]** Every note write now does an embedding call awaited before the write transaction and committed with it when a provider is configured, adding provider latency to writes → accepted because it keeps embeddings consistent with FTS/link-edge updates at the same commit point, and it degrades to "no embedding, write still succeeds" rather than blocking.
