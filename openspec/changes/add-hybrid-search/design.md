...
**Embedding computation: awaited before the write transaction, best-effort.** `SearchIndex.upsert()` (called from the notes-write path) calls and awaits `EmbeddingProvider.embed(content)` _before_ opening the SQLite write transaction — the network round-trip never happens while a transaction is held open. On success, the embedding is included in the same commit as the FTS/link-edge update. On failure (timeout, unreachable, non-2xx) or when no provider is configured, the transaction proceeds without an embedding row for that note — logged, not surfaced as a write error — matching the spec's "embedding computation SHALL NOT block or fail the write response" requirement. The note is queryable via BM25 immediately and picked up by backfill later.
