...
- Cupertino idioms where they matter most on iOS: dialogs, action sheet, pull-to-refresh, no ripples, chevron/horizontal-dots glyphs, swipe-to-delete, status-bar scroll-to-top.
...
- **Swipe-to-delete only on narrow (touch) layouts**, via `Dismissible.confirmDismiss` that runs the same confirmation and returns whether the note was actually removed. The row slides out only on success; a cancelled or failed delete springs it back. Wide layouts already have hover-delete.
