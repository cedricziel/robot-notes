...
/// `POST /notes/files` response: where an uploaded file ended up and how
/// the server sees it.
@immutable
...
class RobotNotesClient {
  ...
  /// `POST /notes/files` — uploads [bytes] into [path] (the vault root
  /// when empty) as [filename]. A filename collision surfaces as
  ...
  Future<UploadedFileResult> uploadFile({
    ...
    final request = http.MultipartRequest('POST', _uri('/notes/files'))
    ...
    return UploadedFileResult.fromJson(_ok(res));
  }
